...
A Claude Code plugin for the **build phase**. Once a plan is set, Claude should
run autonomously to completion, deciding as it goes. When an **unknown unknown**
surfaces — something the plan did not cover, where Claude would normally *stop
and ask* — the plugin instead makes it: **pick the recommended, non-destructive
option → log a Deviations entry → keep going.**

The point is **fewer round-trips**. The companion `clarify-interview` plugin
resolves the *known* unknowns up front; this plugin absorbs the *unknown*
unknowns during the build without a pause.
...
consistent with the `clarify-interview` plugin. Everything smaller is handled
...
| Component | Path | Role |
|---|---|---|
| Hook | `hooks/hooks.json` (+ prompt) | **Enforcement core.** `Stop` prompt-based hook, gated on the notes file existing. |
| Skill | `skills/implementation-notes/SKILL.md` | File format, conservative-choice rule, escape hatch, doc-location discovery, seeding from an existing plan. |
| Command | `commands/notes-start.md` | Manual arm for ad-hoc builds. |
...
- **Activation gate:** no-op unless an `implementation-notes-<datetime>.md`
  exists for the active build. Outside a build, total silence.
- **Judgment:** when armed, evaluate — is Claude ending the turn to *ask a
  ...
  - If yes **and** it is *not* destructive/architecture-altering → block the
    stop; inject "pick the recommended non-destructive option, log it under
    Deviations, and continue."
...
- **File format:** `implementation-notes-<datetime>.md` with at minimum a
...
- **Doc-location discovery:** place the file where the repo keeps docs, to match
...
- **Seeding:** if a plan/spec is present, seed the notes file with an
  "Assumptions carried in" section from it (independent-but-aware coupling).

### Command

- `/notes-start [plan-path]` — manually create the notes file and arm the hook
  for ad-hoc builds. Also the mechanism auto-invoked when a plan/spec is
  approved.

## The file as the activation flag

The **existence** of `implementation-notes-<datetime>.md` is the "we are
building" signal:

- Created when work starts (plan approved, or `/notes-start`).
- While it exists, the `Stop` hook enforces log-and-continue.
...
- If a plan/spec (e.g., from `clarify-interview`) is present, the notes file is
  seeded from it and mid-build surprises can be checked against the assumptions
  already flagged — one continuous paper trail.

## Out of scope (YAGNI)

- No automatic archival/rotation of old notes files in v1.
